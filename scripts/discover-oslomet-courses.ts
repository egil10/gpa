/**
 * Discover and fetch ALL OsloMet courses
...
async function discoverOsloMetCourses() {
  console.log(`
╔════════════════════════════════════════════════════════════╗
║     OsloMet All Courses Discovery & Export (Batched)       ║
╚════════════════════════════════════════════════════════════╝
  `);

  const institutionCode = '1175';
  const institutionName = 'OsloMet';
  ...
  for (let i = 0; i < years.length; i++) {
    ...
    try {
      ...
      // OsloMet uses format COURSECODE-1 (with dash), same as UiO/NTNU
  ...
  const outputFile = path.join(dataDir, 'oslomet-all-courses.json');
...
// Run discovery
discoverOsloMetCourses().catch(console.error);

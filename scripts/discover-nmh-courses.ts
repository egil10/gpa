/**
 * Discover and fetch ALL NMH courses
 * Mirrors the structure used by other institution discovery scripts
 */
...
async function discoverNMHCourses() {
  console.log(`
╔════════════════════════════════════════════════════════════╗
║     NMH All Courses Discovery & Export (Batched)           ║
╚════════════════════════════════════════════════════════════╝
  `);

  const institutionCode = '1210';
  const institutionName = 'NMH';
  ...
  for (let i = 0; i < years.length; i++) {
    ...
    try {
      ...
      // NMH uses the standard COURSECODE-1 API suffix (same as UiO/NTNU/etc.)
  ...
  const outputFile = path.join(dataDir, 'nmh-all-courses.json');
...
// Run discovery
discoverNMHCourses().catch(console.error);

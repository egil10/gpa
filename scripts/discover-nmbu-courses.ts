/**
 * Discover and fetch ALL NMBU courses
 * Uses the same structure as Nord/OsloMet/UiO/UiB/NTNU discovery scripts
 */
...
async function discoverNMBUCourses() {
  console.log(`
╔════════════════════════════════════════════════════════════╗
║     NMBU All Courses Discovery & Export (Batched)          ║
╚════════════════════════════════════════════════════════════╝
  `);

  const institutionCode = '1173';
  const institutionName = 'NMBU';
  ...
  for (let i = 0; i < years.length; i++) {
    ...
    try {
      ...
      // NMBU uses format COURSECODE-1 (with dash), same as UiO/NTNU/OsloMet/Nord
  ...
  const outputFile = path.join(dataDir, 'nmbu-all-courses.json');
...
// Run discovery
discoverNMBUCourses().catch(console.error);

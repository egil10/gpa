/**
 * Discover and fetch ALL USN courses
 * Uses the same structure as UiS/INN/UiA/NMBU/Nord/OsloMet/UiO/UiB/NTNU discovery scripts
 */
...
async function discoverUSNCourses() {
  console.log(`
╔════════════════════════════════════════════════════════════╗
║     USN All Courses Discovery & Export (Batched)           ║
╚════════════════════════════════════════════════════════════╝
  `);

  const institutionCode = '1176';
  const institutionName = 'USN';
  ...
  for (let i = 0; i < years.length; i++) {
    ...
    try {
      ...
      courses.forEach(course => {
        ...
        const baseCode = normalizeCourseCodeForStorage(course.courseCode);
  ...
  const outputFile = path.join(dataDir, 'usn-all-courses.json');
...
// Run discovery
discoverUSNCourses().catch(console.error);

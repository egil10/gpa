/**
 * Discover and fetch ALL Nord universitet courses
 * Uses the same structure as OsloMet/UiO/UiB/NTNU discovery scripts
 */
...
async function discoverNordCourses() {
  console.log(`
╔════════════════════════════════════════════════════════════╗
║     Nord universitet All Courses Discovery & Export        ║
╚════════════════════════════════════════════════════════════╝
  `);

  const institutionCode = '1174';
  const institutionName = 'Nord';
  ...
  for (let i = 0; i < years.length; i++) {
    ...
    try {
      ...
      // Nord uses format COURSECODE-1 (with dash), same as UiO/NTNU/OsloMet
  ...
  const outputFile = path.join(dataDir, 'nord-all-courses.json');
...
// Run discovery
discoverNordCourses().catch(console.error);

/**
 * Discover and fetch ALL UiS courses
 * Uses the same structure as INN/UiA/NMBU/Nord/OsloMet/UiO/UiB/NTNU discovery scripts
 */
...
async function discoverUiSCourses() {
  console.log(`
╔════════════════════════════════════════════════════════════╗
║     UiS All Courses Discovery & Export (Batched)           ║
╚════════════════════════════════════════════════════════════╝
  `);

  const institutionCode = '1160';
  const institutionName = 'UiS';
  ...
  for (let i = 0; i < years.length; i++) {
    ...
    try {
      ...
      // UiS uses format COURSECODE-1 (with dash), same as UiO/NTNU/OsloMet/Nord/NMBU/UiA/INN
      courses.forEach(course => {
        ...
        if (existing) {
          ...
          // Update course name if we have a better one
  ...
  const outputFile = path.join(dataDir, 'uis-all-courses.json');
...
// Run discovery
discoverUiSCourses().catch(console.error);
